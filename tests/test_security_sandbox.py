...
class TestBashSentinelHandling:
    ...
    def test_heredoc_command_completes(self):
        ...
        try:
            result = loop.run_until_complete(session.run(heredoc_cmd))
        finally:
            loop.close()
        assert "hello world" in result.output

    def test_multiline_command_completes(self):
        ...
        loop = asyncio.new_event_loop()
        try:
            result = loop.run_until_complete(session.run(multiline_cmd))
        finally:
            loop.close()
    ...
    def test_sentinel_not_appended_with_semicolon(self):
        ...
        loop = asyncio.new_event_loop()
        try:
            loop.run_until_complete(session.run(test_cmd))
        finally:
            loop.close()
    ...
    def test_incomplete_read_handled(self):
        ...
        def _raise_and_set_rc(*args, **kwargs):
            session._process.returncode = -9
            raise asyncio.IncompleteReadError(partial=b"", expected=16)

        session._process.stdout.readuntil = AsyncMock(side_effect=_raise_and_set_rc)
