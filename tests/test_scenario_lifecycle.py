...
class TestEvalYieldErrorSurfacing:
    """Verify that evaluation errors are properly surfaced, not silently swallowed."""

    @pytest.mark.asyncio
    async def test_error_not_silently_swallowed(self):
        """A scenario that raises during evaluation should produce isError=True."""
        from hud.environment.scenarios import ScenarioMixin, ScenarioSession, _normalize_eval_yield
        ...
        assert result.isError is True or result.reward == 0.0
        if hasattr(result, "isError") and result.isError:
            assert result.isError is True

    @pytest.mark.asyncio
    async def test_error_message_preserved(self):
        """The error message should be accessible from the evaluation result."""
        ...
        assert result is not None
        has_error_info = (
            (hasattr(result, "isError") and result.isError is True)
            or (hasattr(result, "content") and result.content and "error" in str(result.content).lower())
            or result.reward == 0.0
        )
        assert has_error_info

    @pytest.mark.asyncio
    async def test_successful_eval_returns_reward(self):
