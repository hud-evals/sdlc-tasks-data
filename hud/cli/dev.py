...
def run_with_reload(
    ...
    def handle_signal(signum: int, frame: Any) -> None:
        if process:
            ...
            # Gracefully disconnect MCP client before killing child
            process.terminate()
        sys.exit(0)
...
def run_docker_dev_server(
    ...
    # Ad-hoc .env loading for dev mode
    ...
    if watch_paths:
        ...
        for path in watch_paths:
            ...
            if local_path.exists():
                ...
                docker_cmd.extend(["-v", f"{local_path}:{container_path}:rw"])
            else:
    ...
    if debugging_ports:
        ...
        for port_num in debugging_ports:
            docker_cmd.extend(["-p", f"{port_num}:{port_num}"])

    # Append user docker args and image at the end
    if docker_args:
