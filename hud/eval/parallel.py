...
def log_eval_stats(completed: list[EvalContext], context: str = "") -> None:
    ...
    """
    active = [ctx for ctx in completed if ctx.reward is not None]
    rewards = [ctx.reward for ctx in active]
    mean_reward = sum(rewards) / len(active) if active else 0.0
    success_count = sum(1 for ctx in active if ctx.success)
