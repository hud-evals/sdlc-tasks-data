...
from .base import MCPAgent, CategorizedTools
from .openai import OpenAIAgent, OpenAIConfig
from .types import OperatorCreateParams
...
class OperatorAgent(OpenAIAgent):
    ...
    def _categorize_tools(self, tools: list[types.Tool] | None = None) -> CategorizedTools:
        """Old per-agent tool categorization — tries to access spec.role."""
        if tools is None:
        ...
        for tool in tools:
            ...
            if spec:
                if spec.role and spec.role in result.claimed_roles:
                    result.skipped.append((tool, f"role '{spec.role}' conflict"))
                    continue
