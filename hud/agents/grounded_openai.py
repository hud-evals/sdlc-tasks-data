    ...
    async def get_response(self, messages: Any) -> AgentResponse:
        ...
        if not msg.tool_calls:
            return AgentResponse(
                content=msg.content or "", tool_calls=[], done=choice.finish_reason in ("stop", "length"), raw=response,
            )
        ...
        if tc.function.name != "computer":
            return AgentResponse(
                content=f"Error: Model called unexpected tool '{tc.function.name}'", tool_calls=[], done=True, raw=response,
            )
