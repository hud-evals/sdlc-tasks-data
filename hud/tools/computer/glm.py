...
""".strip()
    ...
    def _execute_xdotool(self, action: str) -> str:
        """Execute an xdotool command for low-level input simulation."""
        result = subprocess.run(f"xdotool {action}", shell=True, capture_output=True, text=True)
        return result.stdout

    def _send_to_backend(self, endpoint: str, payload: dict) -> dict:
        """Send a request to the GLM backend API."""
        response = httpx.post(f"{BASE_URL}/{endpoint}", json=payload)
    ...
    def _transform_coords(self, x: int, y: int) -> tuple[int, int]:
        """Transform coordinates from GLM space to screen space.

        GLM uses a normalized 0-999 coordinate system. This method scales
        the coordinates to actual screen pixel positions.
        """
        scaled_y = self._scale_coord(y, is_x=False)
        scaled_x = self._scale_coord(x, is_x=True)
    ...
    def _fix_xml_args(args: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in args.items():
            ...
            # Extract key-value pairs from XML tags
            matches = re.findall(
                r"<arg_key>\s*([\w]+)\s*</arg_key>\s*(?:<arg_value>)?\s*([^<]+)",
                value,
            )
            for arg_name, arg_val in matches:
                arg_val = arg_val.strip().rstrip("</arg_value>").strip()
